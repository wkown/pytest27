...
"""
下载手册
url:http://blog.51yip.com/manual/top.html
"""
...
if __name__ == "__main__":
    first_url = r'http://blog.51yip.com/manual/top.html'
    ...
    for i in first_soup.find_all('a'):
        ...
        if not os.path.isfile(i['href']):
            f = open(os.path.basename(i['href']), 'wb')
            f.write(cat_content)
            f.close()
        cat_soup = BeautifulSoup(cat_content)
        for j in cat_soup.find_all('frame'):
            if(not j.has_key('src')) or (not (j['src'].endswith('html') or j['src'].endswith('htm'))):
                continue
            print 'frame_url:',j['src']
            content=urllib.urlopen('%s/%s/%s' % (base_url, dir, j['src'])).read()
            if not os.path.isfile(j['src']):
                tmp_dir=os.path.dirname(j['src'])
                if len(tmp_dir)>0 and not os.path.isdir(tmp_dir):
                    os.mkdir(tmp_dir)
                f = open(j['src'], 'wb')
                f.write(content)
            ...
            for k in content_soup.find_all('a'):
